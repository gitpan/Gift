...
sub parse_end_of_answer {
  ...
  $inside_answers = $inside_match = $inside_numeric = $answerno = 
  $numright = $weightsum = $num_positive_weights = $inside_truefalse = 0;
  $post_state = 1;
  return ('}', '}');
}
...
sub parse_numeric {
  ...
  if ($input =~ s/\A          # range type {#3.141..3.142}
                  ...
                  (\s*\#((\\.|[^=~}])*))?
                  //x) {  # range
    my ($weight, $firstnumber, $secondnumber, $comment) = ($2, $3, $5, $8); 
    $lineno += &countlines($&);
    ...
    return ('ANSWER', 
            { TYPE  => 'NUMERICRANGE',
              ...
              COMMENT => $comment
                     ...
                     (\s*\#((\\.|[^=~}])*))?
                     //x) { 
    my ($weight, $number, $error, $comment) = ($2, $3, $6, $9);  
    $lineno += &countlines($&);
    $weightsum += $weight if defined($weight);
    &trim_end($comment);
    return ('ANSWER', 
...
sub parse_truefalse {
  ...
  $lineno += &countlines($&);
  my ($token, $commenttrue, $commentfalse) = ($1, $5, $7);
...
sub parse_single_short_answer {
  my $parser = shift;
    ...
    my ($weight, $answer, $comment) = ($2, $3, $6);
    $lineno += &countlines($&);
    if (defined($weight)) {
      $weightsum += $weight;
...
sub parse_match {
  ...
  my ($weight, $comment) = @_;
   ...
   if ((!$inside_match) and ($answerno > 1)) {
     $parser->Error(
...
sub parse_right_or_wrong {
  my $parser = shift;
  my ($type, $weight, $answer, $comment) = @_;
...
sub Lex_inside_answers {
  ...
  return $parser->parse_truefalse() if 
                  ($input =~ s{\A(
  ...
  return $parser->parse_single_short_answer() if 
...
The C<Gift::Question> class provides the method 
